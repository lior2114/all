...
class Users_Model:
    ...
    def show_user_by_email_and_password(user_email, user_password):
        with Users_Model.get_db_connection() as connection:
            ...
            # בדיקה האם הסיסמה מוצפנת בפורמט של Werkzeug
            if isinstance(stored_password, str) and stored_password.startswith("pbkdf2:sha256"):
                try:
                    password_valid = check_password_hash(stored_password, user_password)
                except Exception:
                    password_valid = False
            else:
                # תמיכה בנתונים ישנים: סיסמה בטקסט רגיל
                password_valid = (stored_password == user_password)
                if password_valid:
                    # מיגרציה לסיסמה מוצפנת
                    try:
                        new_hashed = generate_password_hash(user_password)
                        update_sql = "UPDATE users SET user_password = ? WHERE user_id = ?"
                        cursor.execute(update_sql, (new_hashed, user[0]))
                        connection.commit()
                    except Exception:
                        pass
    ...
    def get_user_auth_by_id(user_id):
        with Users_Model.get_db_connection() as connection:
            cursor = connection.cursor()
            sql = "SELECT user_id, user_email, user_password, role_id FROM users WHERE user_id = ?"
            cursor.execute(sql, (user_id,))
            row = cursor.fetchone()
            cursor.close()
            if not row:
