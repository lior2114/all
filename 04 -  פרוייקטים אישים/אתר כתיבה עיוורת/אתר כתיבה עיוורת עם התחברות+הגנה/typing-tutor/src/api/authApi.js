...
class AuthApi {
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 401) {
          throw new Error('יש להתחבר מחדש כדי להציג נתוני התקדמות');
        }
        if (response.status === 403) {
          throw new Error('אין הרשאה לצפות בהתפתחות המשתמש');
        }
        throw new Error(data.Error || data.Message || 'שגיאה בקבלת התקדמות');
      }
  ...
  static async updateUserProgress(progressData) {
    try {
      ...
      if (!response.ok) {
        if (response.status === 401) {
          throw new Error('יש להתחבר מחדש כדי לעדכן התקדמות');
        }
        if (response.status === 403) {
          throw new Error('אין הרשאה לעדכן התקדמות');
        }
        throw new Error(data.Error || data.Message || 'שגיאה בעדכון התקדמות');
      }
