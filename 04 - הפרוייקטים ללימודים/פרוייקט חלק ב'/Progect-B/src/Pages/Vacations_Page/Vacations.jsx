...
export function Vacations() {
  ...
  const [filteredVacations, setFilteredVacations] = React.useState([]);
  ...
  // Filter vacations based on search and filter criteria
  const filterVacations = React.useCallback(() => {
    if (!vacations.length) {
    ...
    if (searchText.trim()) {
      ...
      filtered = filtered.filter(vacation => 
        vacation.country_name?.toLowerCase().includes(searchLower) ||
        vacation.vacation_description?.toLowerCase().includes(searchLower)
      );
    }

    // Price filter
    filtered = filtered.filter(vacation => {
      const price = Number(vacation.vacation_price);
    ...
    if (dateRange.start || dateRange.end) {
      filtered = filtered.filter(vacation => {
        const vacationStart = new Date(vacation.vacation_start);
        const vacationEnd = new Date(vacation.vacation_ends);
        
        if (dateRange.start && dateRange.end) {
          const filterStart = new Date(dateRange.start);
          const filterEnd = new Date(dateRange.end);
          return vacationStart >= filterStart && vacationEnd <= filterEnd;
        } else if (dateRange.start) {
          const filterStart = new Date(dateRange.start);
          return vacationStart >= filterStart;
        } else if (dateRange.end) {
          const filterEnd = new Date(dateRange.end);
          return vacationEnd <= filterEnd;
        }
        return true;
      });
    }
  ...
  }, [vacations, searchText, priceRange, dateRange]);
  ...
  // Initialize max price based on available vacations
  React.useEffect(() => {
    if (vacations.length > 0) {
      const maxPrice = Math.max(...vacations.map(v => Number(v.vacation_price)));
  ...
  return (
    <Container maxWidth="lg">
      ...
      <Paper className={styles.searchFilterSection} data-dark={isDark} sx={{ p: 2, mb: 3 }}>
        <Stack spacing={2}>
          {/* Search Bar */}
          <TextField
            fullWidth
            variant="outlined"
            placeholder={t('חפש לפי מקום החופשה...', 'Search by destination...')}
            value={searchText}
          ...
          {/* Filters Accordion */}
          <Accordion className={styles.filtersAccordion} data-dark={isDark}>
            <AccordionSummary expandIcon={<ExpandMore />}>
              <Stack direction="row" alignItems="center" spacing={1}>
                <FilterList />
                <Typography variant="h6">{t('מסננים', 'Filters')}</Typography>
              ...
              <Stack spacing={3}>
                ...
                <Box>
                  ...
                  <Slider
                    ...
                    min={0}
                    max={vacations.length ? Math.max(...vacations.map(v => Number(v.vacation_price))) : 10000}
                    step={100}
                    marks={[
                      { value: 0, label: '₪0' },
                      { value: vacations.length ? Math.max(...vacations.map(v => Number(v.vacation_price))) : 10000, 
                        label: `₪${vacations.length ? Math.max(...vacations.map(v => Number(v.vacation_price))).toLocaleString() : '10,000'}` }
                    ]}
                    className={styles.priceSlider}
                  />
                  <Typography variant="body2" color="text.secondary">
                    {t(`₪${priceRange[0].toLocaleString()} - ₪${priceRange[1].toLocaleString()}`, 
                ...
                <Box sx={{ display: 'flex', justifyContent: 'flex-end' }}>
                  <Button 
                    ...
                    disabled={!searchText && !dateRange.start && !dateRange.end}
                  >
                    {t('נקה מסננים', 'Clear Filters')}
                  </Button>
                </Box>
              </Stack>
            </AccordionDetails>
      ...
      {!loading && vacations.length > 0 && (
        <Box sx={{ mb: 2, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <Typography variant="body2" color="text.secondary">
            {filteredVacations.length === vacations.length 
              ? t(`מציג ${vacations.length} חופשות`, `Showing ${vacations.length} vacations`)
              : t(`מציג ${filteredVacations.length} מתוך ${vacations.length} חופשות`, 
      ...
      {loading ? (
        ...
        <Paper className={styles.noResultsContainer} data-dark={isDark}>
          <Typography variant="h6" gutterBottom>
            {t('לא נמצאו חופשות המתאימות לקריטריונים', 'No vacations match your criteria')}
          </Typography>
          <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
            {t('נסה לשנות את הקריטריונים או נקה את המסננים', 'Try changing your criteria or clear the filters')}
          </Typography>
          <Button 
            variant="contained" 
            onClick={clearFilters}
              ...
              <Box key={vacationId} role="listitem" className={styles.listItem}>
                <Card className={styles.card}>
                  ...
                  <CardContent className={styles.cardContent}>
                    ...
                    <Button aria-label={t('מחיר חופשה', 'Vacation price')} variant="contained" fullWidth sx={{ fontWeight: 'bold' }}>
                  ...
                  {!isAdmin && (
                    <CardActions className={styles.actionsRow}>
                      <Tooltip title={canLike ? (isLiked ? t('בטל לייק', 'Unlike') : t('לייק', 'Like')) : t('יש להתחבר כדי לעשות לייק', 'Login to like')}>
                        <span>
                          <IconButton
