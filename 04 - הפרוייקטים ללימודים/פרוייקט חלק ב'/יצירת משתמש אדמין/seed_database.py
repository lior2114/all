...
def upsert_vacations(conn: sqlite3.Connection) -> None:
    ...
    def add_vac(country: str, desc: str, start: date, days: int, price: float, file_name: str = 'https://example.com/placeholder.jpg'):
        ...
        if cur.fetchone():
            return
        cur.execute(
            '''INSERT INTO vacations (country_id, vacation_description, vacation_start, vacation_ends, vacation_price, vacation_file_name)
               VALUES (?, ?, ?, ?, ?, ?)''',
            (cid, desc, start.isoformat(), end.isoformat(), float(price), file_name)
        )

    base = date.today() + timedelta(days=15)
    add_vac('Israel',   'Vacation in Tel Aviv. Includes beaches and markets.', base, 7, 10000)
    add_vac('Greece',   'Romantic vacation in Santorini. Sunsets and beaches.', base + timedelta(days=14), 7, 5500)
    add_vac('Italy',    'Explore Rome: Colosseum, Vatican, Trevi Fountain.', base + timedelta(days=28), 8, 4500)
    add_vac('France',   'Paris highlights: Eiffel Tower, Louvre, Montmartre.', base + timedelta(days=42), 7, 6500)
    add_vac('Spain',    'Barcelona: Sagrada Família, Park Güell, Gothic Quarter.', base + timedelta(days=56), 7, 5200)
    add_vac('Turkey',   'Istanbul and Bosphorus tour with bazaars and cuisine.', base + timedelta(days=70), 6, 3900)
    add_vac('Cyprus',   'Relax in Paphos seaside resorts and ancient sites.', base + timedelta(days=84), 6, 3400)
    add_vac('Japan',    'Tokyo and Kyoto culture, gardens and temples.', base + timedelta(days=98), 9, 8200)
    add_vac('Thailand', 'Phuket beaches and island hopping adventure.', base + timedelta(days=112), 8, 5700)
    add_vac('United States', 'New York & Miami city experience and beaches.', base + timedelta(days=126), 10, 9200)
    add_vac('Brazil',   'Rio de Janeiro: Copacabana, Sugarloaf, Christ the Redeemer.', base + timedelta(days=140), 8, 6100)
    add_vac('Morocco',  'Marrakesh souks and desert trip to Atlas foothills.', base + timedelta(days=154), 7, 4300)
