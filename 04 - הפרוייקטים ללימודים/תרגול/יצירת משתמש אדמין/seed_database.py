...
- At least 12 vacations with reasonable data
- Likes table emptied
...
def upsert_users(conn: sqlite3.Connection) -> None:
    ...
    if not get_user_by_email(conn, 'user@test.com'):
        cur.execute(
            'INSERT INTO users (first_name, last_name, user_email, user_password, role_id) VALUES (?, ?, ?, ?, ?)',
            ('Demo', 'User', 'user@test.com', 'user1234', 2)
        )
    conn.commit()
...
def upsert_vacations(conn: sqlite3.Connection) -> None:
    ...
    def add_vac(country: str, name: str, desc: str, start: date, days: int, price: float, file_name: str = 'https://example.com/placeholder.jpg'):
        cid = name_to_id.get(country.lower())
        if not cid:
            return
        end = start + timedelta(days=days)
        cur.execute(
            '''SELECT 1 FROM vacations WHERE country_id=? AND vacation_name=? AND vacation_description=? AND vacation_start=? AND vacation_ends=?''',
            (cid, name, desc, start.isoformat(), end.isoformat())
        )
        if cur.fetchone():
            return
        cur.execute(
            '''INSERT INTO vacations (country_id, vacation_name, vacation_description, vacation_start, vacation_ends, vacation_price, vacation_file_name)
               VALUES (?, ?, ?, ?, ?, ?, ?)''',
            (cid, name, desc, start.isoformat(), end.isoformat(), float(price), file_name)
        )

    base = date.today() + timedelta(days=15)
    add_vac('Israel',   'Tel Aviv Beach Adventure', 'Vacation in Tel Aviv. Includes beaches and markets.', base, 7, 10000)
    add_vac('Greece',   'Santorini Sunset Romance', 'Romantic vacation in Santorini. Sunsets and beaches.', base + timedelta(days=14), 7, 5500)
    add_vac('Italy',    'Rome Historical Journey', 'Explore Rome: Colosseum, Vatican, Trevi Fountain.', base + timedelta(days=28), 8, 4500)
    add_vac('France',   'Paris City of Lights', 'Paris highlights: Eiffel Tower, Louvre, Montmartre.', base + timedelta(days=42), 7, 6500)
    add_vac('Spain',    'Barcelona Art & Culture', 'Barcelona: Sagrada Família, Park Güell, Gothic Quarter.', base + timedelta(days=56), 7, 5200)
    add_vac('Turkey',   'Istanbul Bosphorus Tour', 'Istanbul and Bosphorus tour with bazaars and cuisine.', base + timedelta(days=70), 6, 3900)
    add_vac('Cyprus',   'Paphos Seaside Retreat', 'Relax in Paphos seaside resorts and ancient sites.', base + timedelta(days=84), 6, 3400)
    add_vac('Japan',    'Tokyo & Kyoto Experience', 'Tokyo and Kyoto culture, gardens and temples.', base + timedelta(days=98), 9, 8200)
    add_vac('Thailand', 'Phuket Island Paradise', 'Phuket beaches and island hopping adventure.', base + timedelta(days=112), 8, 5700)
    add_vac('United States', 'NYC & Miami Adventure', 'New York & Miami city experience and beaches.', base + timedelta(days=126), 10, 9200)
    add_vac('Brazil',   'Rio de Janeiro Highlights', 'Rio de Janeiro: Copacabana, Sugarloaf, Christ the Redeemer.', base + timedelta(days=140), 8, 6100)
    add_vac('Morocco',  'Marrakesh Desert Escape', 'Marrakesh souks and desert trip to Atlas foothills.', base + timedelta(days=154), 7, 4300)
