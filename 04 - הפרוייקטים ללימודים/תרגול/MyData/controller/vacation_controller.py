...
class Vacations_Controller:
    ...
    def create_vacation():
        ...
        # Convert country_id to int if needed
        if data.get("country_id"):
            try:
                data["country_id"] = int(data["country_id"])
            except (ValueError, TypeError):
                return jsonify({"Error": "Invalid country_id format"}), 400

        # allow missing vacation_file_name; require either country_id or country_name
        base_fields = ["vacation_name", "vacation_description", "vacation_start", "vacation_ends", "vacation_price"]
        ...
        # Admin info is available in request.admin_user and request.admin_id
        ...
        # Convert vacation_price to float for validation
        try:
            price = float(data["vacation_price"])
            data["vacation_price"] = price  # Update data with converted value
        except (ValueError, TypeError):
            return jsonify({"Error": "Invalid vacation_price format"}), 400
        ...
        result = V.create_vacation(
            ...
            vacation_name = data["vacation_name"],
