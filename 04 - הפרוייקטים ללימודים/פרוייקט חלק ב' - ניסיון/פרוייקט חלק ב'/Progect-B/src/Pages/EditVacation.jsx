...
export function EditVacation() {
  ...
  // Load existing vacation data and countries
  React.useEffect(() => {
    const load = async () => {
      try {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      // ensure we have a country_id (create if necessary)
  ...
  return (
    <Container maxWidth="md" className={styles.editVacationContainer}>
      <Paper className={styles.editVacationPaper}>
        <Typography variant="h4" gutterBottom className={styles.editVacationTitle}>עריכת חופשה #{id}</Typography>
        {error && <Alert severity="error" className={styles.errorAlert}>{error}</Alert>}
        <Box component="form" onSubmit={onSubmit} className={styles.editVacationForm}>
          <Grid container spacing={2} className={styles.formGrid}>
            <Grid item xs={12} sm={6} className={styles.countryField}>
              <TextField
                fullWidth
              ...
              />
              {panelOpen && (
                <Box id="countries-edit-list" role="listbox" aria-label="רשימת מדינות" className={styles.countryDropdown}>
                  {filteredCountries
                    .slice(0, 200)
                    .map((c,idx)=> {
            ...
            <Grid item xs={12} sm={6} className={styles.priceField}>
              <TextField
                fullWidth
                label={t('מחיר', 'Price')}
                name="vacation_price"
                ...
                InputProps={{
                  startAdornment: (
                    <InputAdornment position="start">
                      <Box className={styles.currencySymbol}>{currencySymbol}</Box>
                    </InputAdornment>
                  ),
                }}
                required
              />
              <Box className={styles.currencyButtons}>
                <Button size="small" className={styles.currencyButton} variant={currency==='ILS'?'contained':'outlined'} onClick={()=> setCurrency('ILS')}>₪ ILS</Button>
                <Button size="small" className={styles.currencyButton} variant={currency==='USD'?'contained':'outlined'} onClick={()=> setCurrency('USD')}>$ USD</Button>
                <Button size="small" className={styles.currencyButton} variant={currency==='EUR'?'contained':'outlined'} onClick={()=> setCurrency('EUR')}>€ EUR</Button>
              </Box>
            </Grid>
            <Grid item xs={12} className={styles.descriptionField}>
            ...
            <Grid item xs={12} sm={6} className={styles.dateFields}>
            ...
            <Grid item xs={12} className={styles.fileUpload}>
              ...
              <Box className={styles.imageOptions}>
                ...
                {currentImage ? (
                  <Typography variant="caption" className={styles.currentImageInfo}>{t('תמונה נוכחית:', 'Current image:')} {String(currentImage).slice(0, 60)}</Typography>
                ) : (
                  <Typography variant="caption" className={styles.currentImageInfo}>{t('אין תמונה — תוצג תמונת דיפולט לפי האזור', 'No image — will show region default')}</Typography>
                )}
              </Box>
            </Grid>
            <Grid item xs={12}>
              <Button type="submit" variant="contained" disabled={submitting} className={styles.submitButton}>שמור</Button>
            </Grid>
          </Grid>
        </Box>
      </Paper>
