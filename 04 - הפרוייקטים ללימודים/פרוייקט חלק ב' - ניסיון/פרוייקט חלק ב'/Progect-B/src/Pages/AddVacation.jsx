...
export function AddVacation() {
  ...
  return (
    <Container maxWidth="md" className={styles.addVacationContainer}>
      <Paper className={styles.addVacationPaper}>
        <Typography variant="h4" gutterBottom className={styles.addVacationTitle}>{t('הוספת חופשה', 'Add Vacation')}</Typography>
        {error && <Alert severity="error" className={styles.errorAlert}>{error}</Alert>}
        <Box component="form" onSubmit={onSubmit} className={styles.addVacationForm}>
          <Grid container spacing={2} className={styles.formGrid}>
            <Grid item xs={12} sm={6} className={styles.countryField}>
              ...
              />
              {panelOpen && (
                <Box id="countries-add-list" role="listbox" aria-label="רשימת מדינות" className={styles.countryDropdown}>
                  {worldCountries
                    .filter((c)=> {
            ...
            <Grid item xs={12}>
              <Button type="submit" variant="contained" disabled={submitting} className={styles.submitButton}>{t('שמור', 'Save')}</Button>
            </Grid>
          </Grid>
        </Box>
      </Paper>
