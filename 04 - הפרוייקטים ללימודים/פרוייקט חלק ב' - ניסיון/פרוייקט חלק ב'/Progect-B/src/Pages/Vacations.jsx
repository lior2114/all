...
import { Add, Favorite, FavoriteBorder, Edit, Delete, CalendarToday } from '@mui/icons-material';
...
export function Vacations() {
  ...
  return (
    <Container maxWidth="lg">
      <Box className={styles.vacationsContainer}>
        <Typography variant="h4" component="h1" className={styles.vacationsTitle}>{t('חופשות', 'Vacations')}</Typography>
        {isAdmin && (
          <Button variant="contained" startIcon={<Add />} onClick={() => navigate('/vacations/add')} className={styles.addButton}>
      ...
      {loading ? (
        ...
        <Box role="list" className={styles.vacationsGrid}>
          {vacations.map((vacation) => {
            ...
            return (
              <Box key={vacationId} role="listitem" sx={{ display: 'flex' }}>
                  ...
                  <Box className={styles.vacationImageContainer}>
                    ...
                    {isAdmin && (
                      <Stack direction="row" spacing={1} className={styles.adminActions}>
                        <Button aria-label={t('עריכה', 'Edit')} size="small" variant="outlined" color="inherit" onClick={() => navigate(`/vacations/edit/${vacationId}`)} startIcon={<Edit />} className={styles.adminButton}>Edit</Button>
                        <Button aria-label={t('מחיקה', 'Delete')} size="small" variant="outlined" color="inherit" onClick={() => setConfirmDeleteId(vacationId)} startIcon={<Delete />} className={styles.adminButton}>Delete</Button>
                      </Stack>
                    )}
                    {/* title overlay */}
      ...
      <Dialog open={Boolean(confirmDeleteId)} onClose={() => setConfirmDeleteId(null)} className={styles.deleteDialog}>
        <DialogTitle className={styles.deleteDialogTitle}>{t('אישור מחיקה', 'Confirm Delete')}</DialogTitle>
