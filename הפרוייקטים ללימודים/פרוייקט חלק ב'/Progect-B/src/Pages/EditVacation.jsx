        ...
        <Box component="form" onSubmit={onSubmit}>
          <Grid container spacing={2}>
            <Grid item xs={12} sm={6} sx={{ position: 'relative' }}>
              ...
              />
              {panelOpen && (
                <Box id="countries-edit-list" role="listbox" aria-label="רשימת מדינות" sx={{ maxHeight: 300, overflowY: 'auto', border: 1, borderColor: 'divider', borderRadius: 1, mt: 1, backgroundColor: 'background.paper', position: 'absolute', zIndex: 10, width: '100%' }}>
                  {filteredCountries
                    .slice(0, 200)
                    .map((c,idx)=> {
                      const match = countries.find((db)=> String(db.country_name).toLowerCase() === (language==='he' ? (c.name_he||'').toLowerCase() : (c.name_en||'').toLowerCase()));
                      return (
                        <Box key={`${c.code}-${idx}`} role="option" onMouseDown={(e)=> e.preventDefault()} onClick={()=> { setForm((p)=> ({...p, country_id: match?.country_id || '' })); setSelectedCountry(c); setCountryQuery(language==='he' ? c.name_he : c.name_en); setPanelOpen(false); }} sx={{ px: 1.5, py: 1, cursor: 'pointer', '&:hover': { bgcolor: 'action.hover' }, borderBottom: 1, borderColor: 'divider' }}>
            ...
            <Grid item xs={12} sm={6}>
                ...
                InputProps={{
                  startAdornment: (
                    <InputAdornment position="start">
                      <Box sx={{ display: 'inline-block', width: 18, textAlign: 'center' }}>{currencySymbol}</Box>
                    </InputAdornment>
                  ),
                }}
                required
              ...
              <Box sx={{ mt: 1, display: 'flex', gap: 1 }}>
                <Button size="small" sx={{ minWidth: 86 }} variant={currency==='ILS'?'contained':'outlined'} onClick={()=> setCurrency('ILS')}>₪ ILS</Button>
                <Button size="small" sx={{ minWidth: 86 }} variant={currency==='USD'?'contained':'outlined'} onClick={()=> setCurrency('USD')}>$ USD</Button>
                <Button size="small" sx={{ minWidth: 86 }} variant={currency==='EUR'?'contained':'outlined'} onClick={()=> setCurrency('EUR')}>€ EUR</Button>
              </Box>
            </Grid>
            <Grid item xs={12}>
