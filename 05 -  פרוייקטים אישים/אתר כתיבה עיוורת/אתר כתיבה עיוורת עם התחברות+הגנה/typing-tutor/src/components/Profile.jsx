...
const Profile = () => {
  ...
  const renderRecentLesson = (lesson, idx) => {
    ...
    const title = (() => {
      if (lesson.lesson_type === 'lesson') {
        return `${isRTL ? 'שיעור' : 'Lesson'} ${lesson.lesson_id}`;
      }
    ...
    return (
      <div key={`${lesson.lesson_id}-${lesson.completed_at || idx}`} className={`recent-lesson-item ${isOpen ? 'open' : ''}`} style={{
        ...
        {isOpen && (
          <div className="recent-lesson-details" style={{ marginTop: '10px', display: 'grid', gridTemplateColumns: 'repeat(4, minmax(0, 1fr))', gap: '10px' }}>
            <div className="detail-box" style={{ background: 'rgba(255,255,255,0.04)', padding: '10px', borderRadius: '8px' }}>
              <div style={{ opacity: 0.8 }}>{isRTL ? 'שעה' : 'Time'}</div>
              <div style={{ fontWeight: 700 }}>{timeOnly || '-'}</div>
            </div>
            <div className="detail-box" style={{ background: 'rgba(255,255,255,0.04)', padding: '10px', borderRadius: '8px' }}>
              <div style={{ opacity: 0.8 }}>{isRTL ? 'מילים לדקה' : 'WPM'}</div>
              <div style={{ fontWeight: 700 }}>{Math.round(lesson.wpm || 0)}</div>
            </div>
            <div className="detail-box" style={{ background: 'rgba(255,255,255,0.04)', padding: '10px', borderRadius: '8px' }}>
              <div style={{ opacity: 0.8 }}>{isRTL ? 'דיוק' : 'Accuracy'}</div>
              <div style={{ fontWeight: 700 }}>{Math.round(lesson.accuracy || 0)}%</div>
            </div>
            <div className="detail-box" style={{ background: 'rgba(255,255,255,0.04)', padding: '10px', borderRadius: '8px' }}>
              <div style={{ opacity: 0.8 }}>{isRTL ? 'שגיאות' : 'Errors'}</div>
              <div style={{ fontWeight: 700 }}>{lesson.errors || 0}</div>
            </div>
            <div className="detail-box" style={{ background: 'rgba(255,255,255,0.04)', padding: '10px', borderRadius: '8px' }}>
              <div style={{ opacity: 0.8 }}>{isRTL ? 'זמן' : 'Time'}</div>
              <div style={{ fontWeight: 700 }}>{formatTime(lesson.time_spent || 0)}</div>
            </div>
          </div>
        )}
      </div>
  ...
  // האזן לאירוע גלובלי שמודיע שהתבצע עדכון התקדמות, ורענן מיד
  useEffect(() => {
    ...
    return () => window.removeEventListener('user-progress-updated', onProgressUpdated);
  }, [user]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // אימות סיסמה נוכחית מול השרת לפני כל עדכון
      try {
        await AuthApi.login(user.user_email, formData.currentPassword);
      } catch (authErr) {
        const msg = isRTL ? 'סיסמה נוכחית לא נכונה' : 'Current password is incorrect';
        setValidationErrors(prev => ({ ...prev, currentPassword: msg }));
        // מניעת הצגת שגיאת "התחברות נכשלה" הכללית של הקונטקסט
        clearError();
        setIsLoading(false);
        return;
      }
      ...
      if (formData.email !== user.user_email) {
        // בדיקת מייל תפוס לפני שליחת העדכון
        const exists = await AuthApi.checkEmailExists(formData.email);
        if (exists) {
          setValidationErrors(prev => ({ ...prev, email: isRTL ? 'האימייל כבר קיים במערכת' : 'Email already exists' }));
          setIsLoading(false);
          return;
        }
